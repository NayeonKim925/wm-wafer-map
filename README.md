...
| ✅ **Tested + CI** | 80+ `pytest` tests on synthetic fixtures (no download), incl. a no-lot-leakage assertion; ruff + pytest on 3.10–3.12 in CI. |

> **About the figures below.** They come from a real run of this pipeline on the
> full labelled **WM-811K** dataset (172,950 wafers, leakage-free `lot` split,
> `wafer_cnn`, seed 42) — the same run written up in
> [`docs/EXAMPLE_REPORT.md`](docs/EXAMPLE_REPORT.md). Reproduce with `python train.py`.
...
Trained on the full labelled **WM-811K** dataset (172,950 wafers, leakage-free
`lot` split, `wafer_cnn`, seed 42). Metrics on the held-out test split (25,824
wafers, 9 classes):

| Macro-F1 | Accuracy | Balanced accuracy | Weighted-F1 | Cohen's κ |
| :---: | :---: | :---: | :---: | :---: |
| **0.837** | **0.961** | **0.902** | 0.964 | 0.863 |

Balanced accuracy stays at 0.902 despite the ~85% `none` majority; per-class F1
ranges from 0.98 (`Edge-Ring`, `none`) down to 0.60 (`Scratch`). Full per-class
precision/recall/F1 and the run config are in
...
├── tests/              # pytest suite (synthetic fixtures, kagglehub mocked)
...
pytest            # 80+ tests on synthetic fixtures, kagglehub mocked
ruff check .
```
